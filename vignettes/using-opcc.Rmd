...
| Rebuild every published artifact from raw public inputs | [Part 2](#part-2-reproduce-all-artifacts-from-scratch) -- requires a source checkout and ~1.6 GB of public inputs; build scripts are not installed with the package |
...
## Part 2: Reproduce all artifacts from scratch

This section rebuilds every published OPCC artifact from public source inputs.
It is a maintainer or auditor workflow; package users do not need it.

The pipeline has three stages:

```text
M1: raw sources --> postal centroids --> DB assignment --> GAF roll-up
M2: GAF roll-up --> postal-code-to-DB correspondence
M5: DB correspondence --> postal-code-to-DA correspondence
```

### Prerequisites

The build requires `sf`, which needs system libraries GDAL, GEOS, and PROJ.
...
Clone the repository and work from its root. All large inputs go to
`.scratch/` (git-ignored) and must not be committed.
...
Download and profile the Statistics Canada National Address Register (NAR) and
the GeoNames reference layer:
...
### Stage 3: M5 -- postal-code-to-DA correspondence

M5 is a deterministic attribute roll-up from the versioned M2 artifact. It
needs no new boundary download or spatial join.

```bash
Rscript scripts/m5_build_da_correspondence.R
```

Output is written to `releases/m5/2026-06-26/`. Verify:
...
From any checkout, verify the immutable public artifacts against their
commit-pinned checksums:
