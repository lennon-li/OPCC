...
.render_opcc_reproducer_script <- function(input_file, postal_col, output_dir,
  ...
  paste0(
    ...
    "postal_centroids <- OPCC:::.load_postal_centroids()\n",
    "postal_points <- OPCC:::.filter_postal_centroids(postal_centroids, codes)\n",
    "if (nrow(postal_points) == 0L) {\n",
    "  message(\"No postal code points available; skipping opcc_postal_points.zip.\")\n",
    "} else {\n",
    "  points_sf <- OPCC:::.postal_points_sf(postal_points, joined)\n",
    "  OPCC:::.write_postal_points_shapefile(\n",
    "    points_sf, file.path(output_dir, \"opcc_postal_points.zip\"))\n",
    "}\n",
    "\n",
