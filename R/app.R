...
#' @export
run_app <- function(...) {
  # Checked with requireNamespace() rather than rlang::check_installed():
  # rlang is itself only suggested, so using it here would report the wrong
  # missing package on a machine that has neither.
  needed <- c("shiny", "bslib", "DT", "leaflet", "htmlwidgets", "sf")
  missing <- needed[!vapply(needed, requireNamespace, logical(1), quietly = TRUE)]
  if (length(missing) > 0L) {
    stop(
      "The OPCC Shiny app requires: ", paste(missing, collapse = ", "),
  ...
  if (utils::packageVersion("bslib") < "0.6.0") {
    stop("The OPCC Shiny app requires bslib >= 0.6.0; installed: ",
         utils::packageVersion("bslib"), call. = FALSE)
  }
  ...
  # no ::: calls: the internals are already in scope.
  app_env <- new.env(parent = asNamespace("OPCC"))
  app <- source(app_file, local = app_env)$value
  if (!inherits(app, "shiny.appobj")) {
    stop("The OPCC Shiny app file did not return a shiny app object.", call. = FALSE)
  }
  shiny::runApp(app, ...)
}
...
.render_opcc_reproducer_script <- function(input_file, postal_col, output_dir,
  ...
  intro <- if (use_codes) {
    paste0(
      "# Reproduces opcc_postal_da.csv, opcc_postal_points.zip, and opcc_map.html\n",
      "# as produced by the OPCC postal-code-to-dissemination-area Shiny app\n",
      "# (OPCC::run_app()), for postal codes typed directly into the app.\n",
      "#\n",
  ...
  paste0(
    ...
    "attr(joined, \"opcc_postal_code_col\") <- opcc_code_col\n",
    "attr(joined, \"opcc_dauid_col\") <- da_col\n",
    "\n",
    ...
    "row.names = FALSE)\n",
    "\n",
    "# Point shapefile of the user's postal codes, one feature per unique code,\n",
    "# with the DA rollup carried as attributes (EPSG:4326), from the same postal\n",
    "# centroid points the app draws on its map. Fetch the points once and reuse\n",
    "# them; codes without any point evidence cannot appear.\n",
    "written <- OPCC::export_postal_points(\n",
    "  codes, joined, file.path(output_dir, \"opcc_postal_points.zip\"))\n",
    "if (is.null(written)) {\n",
    "  message(\"No postal code points available; skipping opcc_postal_points.zip.\")\n",
    "}\n",
    "\n",
...
#' Write a postal-code point shapefile for an OPCC join
#'
#' Builds a point layer for the supplied postal codes, one feature per unique
#' code, carrying the dissemination-area roll-up from `joined` as attributes,
#' and writes it to a zipped ESRI Shapefile. The points are the same
#' source-qualified postal centroids the OPCC Shiny app draws on its map, so a
#' script generated by the app reproduces exactly what the app displayed.
#'
#' Codes with no point evidence cannot appear in the output; that is a property
#' of the evidence, not a failure. When no supplied code has any point
#' evidence, nothing is written and the function returns `NULL`.
#'
#' @param postal_code Character vector of postal codes.
#' @param joined A data frame produced by joining records to an OPCC
#'   correspondence, carrying the `opcc_postal_code_col` and `opcc_dauid_col`
#'   attributes set by the app. Plain `opcc_postal_code` and `DAUID` columns are
...
#' }
#' }
export_postal_points <- function(postal_code,
                                 joined,
                                 zipfile,
                                 vintage = "2026-06-26",
                                 cache_dir = tools::R_user_dir("OPCC", "cache"),
                                 offline = FALSE) {
  points <- .postal_centroids(postal_code, vintage, NULL, cache_dir, offline)
  if (nrow(points) == 0L) {
    return(NULL)
  }
  .write_postal_points_shapefile(.postal_points_sf(points, joined), zipfile)
}
