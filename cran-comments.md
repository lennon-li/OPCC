...
- Package test suite under check: 484 passing, 13 skipped, 0 failures. The
...
- `R CMD check --as-cran` with `_R_CHECK_DEPENDS_ONLY_=true`:
  `Status: 1 NOTE`, the same new-submission NOTE; 405 passing, 30 skipped,
  0 failures.
- With all suggested packages installed, the full local suite runs 544 passing,
...
GitHub Actions run 32756210069 checked commit 4fe519e on
ubuntu-latest, macos-latest, and windows-latest with
`rcmdcheck::rcmdcheck(args = c("--no-manual", "--as-cran"))`, plus an Ubuntu
release-validation job. All four jobs passed with 0 errors, 0 warnings, and the
new-submission NOTE. The Windows log contains no reparse-point cleanup warning.
